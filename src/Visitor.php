...
class Visitor implements Countable{
	...
	*/
    ...
    public function log($article_id)
	{
		...
		if( $this->has( $ip ) && $this->hasArticle($ip, $article_id))
		{
			//ip already exist in db.
			$this->storage->increment( $ip, $article_id );
		}
		else
		{
	...
	public function hasArticle($ip, $id)
	{
		return $this->storage->countArticleIp($ip, $id);
	}
